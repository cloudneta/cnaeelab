...
## 2. EKS Fully Private Cluster 배포

<br/>

### 2.1. EKS Fully Private Cluster 배포

Amazon EKS Fully Private Cluster를 생성하기 위해 eksctl 명령을 활용해 생성합니다.  
...
{: .box-note}
**Note:** 참고로 eksctl로 private access를 구성하면 바로 private 환경으로 구성되지 않고 public & private 환경으로 설정 후 자동으로 public access를 제거합니다.

{: .box-warning}
**Warning:** EKS Fully Cluster 구성을 할 때 보안 그룹 설정이 필요합니다. 클러스터 배포가 진행 중일 때 빠른 시점에 아래 보안 그룹 생성 작업을 진행해 주세요. 늦게 설정하면 클러스터 배포가 실패 될 것입니다!!
