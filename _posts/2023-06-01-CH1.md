...
[해당 링크](https://console.aws.amazon.com/cloudformation/home?region=ap-northeast-2#/stacks/new?stackName=myeks&templateURL=https:%2F%2Fs3.ap-northeast-2.amazonaws.com%2Fcloudformation.cloudneta.net%2FK8S%2Fmyeks-1week.yaml){:target="_blank"}를 클릭하면 AWS CloudFormation 페이지로 연결되며, 파라미터를 입력(키 페어, 작업 PC의 공인 IP 주소/32) 후 스택을 실행합니다.
...
AWS CloudFormation 스택의 출력 탭에서 eksctlhost의 퍼블릭 IP를 확인하고 EKS 관리용 인스턴스(`myeks-host`)에 SSH로 접속하고 아래 명령어를 통해 정보를 확인합니다.
...
관리 콘솔로 EKS 클러스터 IAM 역할을 사용하기 위해 클러스터 OIDC에 IAM OIDC 제공업체가 존재해야 되서 아래와 같은 작업을 수행합니다.
{% highlight javascript linenos %}
// OIDC ID 값 확인
...
<span style='color:white; background-color:#404040'> **보안 그룹 설정 및 Ping 테스트** </span>

{% highlight javascript linenos %}
// Ping 테스트 (N1, N2) --> 통신 불가
ping -c 2 $N1

ping -c 2 $N2

// 노드 그룹의 보안 그룹 ID 확인 및 변수 지정
...
// myeks-host에서 워커 노드로 통신 가능한 보안 그룹 정책 추가
...
ssh -i ec2-user@$N1 hostname

ssh -i ec2-user@$N2 hostname
{% endhighlight %}

<br/>

<span style='color:white; background-color:#404040'> **워커 노드의 네트워크 구성 확인** </span>

{% highlight javascript linenos %}
// kube-system 네임 스페이스의 파드 정보 확인
kubectl get pod -n kube-system -o wide

// 노드 정보 확인
kubectl get node -owide

// 워커 노드 SSH 접속하여 IP 및 라우트 정보 확인
ssh -i ec2-user@$N1 sudo ip -c addr

ssh -i ec2-user@$N2 sudo ip -c addr

ssh -i ec2-user@$N1 sudo ip -c route

ssh -i ec2-user@$N2 sudo ip -c route
{% endhighlight %}
...
{% highlight javascript linenos %}
// kubelet 상태 정보
ssh -i ec2-user@$N1 sudo systemctl status kubelet

ssh -i ec2-user@$N2 sudo systemctl status kubelet

// 프로세스 확인
ssh -i ec2-user@$N1 sudo pstree
ssh -i ec2-user@$N1 sudo ps afxuwww

ssh -i ec2-user@$N2 sudo pstree
ssh -i ec2-user@$N2 sudo ps afxuwww

// 컨테이너 런타임 확인
ssh -i ec2-user@$N1 sudo ps axf |grep /usr/bin/containerd

ssh -i ec2-user@$N2 sudo ps axf |grep /usr/bin/containerd
{% endhighlight %}
...
{% highlight javascript linenos %}
// 스토리지 정보
ssh -i ec2-user@$N1 lsblk

ssh -i ec2-user@$N2 lsblk
{% endhighlight %}
...
ssh -i ec2-user@$N1 sudo ss -tnp
ssh -i ec2-user@$N2 sudo ss -tnp
...
curl -s -O https://github.com/cloudneta/cnaeblab/blob/master/_data/mario.yaml
