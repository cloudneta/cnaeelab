...
subtitle: EKS Ingress ALB 환경에서 Rolling Update 수행 시 트래픽 유실 없는 서비스를 구성한다.
tags: [eks, 2장]
published: true
---
|목차|
...
while true; do ./req_access.sh; sleep 0.01; done


// [신규 터미널2] Web 접속 스크립트 (무한) - HTTPCode 200 OK 제외
cd demo_ch2_2

while true; do ./req_access.sh | grep -v "=200"; sleep 0.01; done
...
<span style='color:white; background-color:#404040'> **모니터링 변경** </span>

{% highlight javascript linenos %}
// [신규 터미널3] ALB Taget Group - IP, State
while true; do date; aws elbv2 describe-target-health \
  --target-group-arn $ELB_TG_ARN \
  --query 'TargetHealthDescriptions[*].{IP: Target.Id, State: TargetHealth.State}' \
  --output text; sleep 1; done
...
2장 세 번째 실습을 이어서 진행하려면 현재 생성 자원을 유지합니다.  
